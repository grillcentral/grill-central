...
export default function Home() {
  ...
  // Carrega overrides do Supabase ao montar e quando admin salvar
  useEffect(() => {
  ...
  // Aplica overrides do Supabase e filtra ocultos
    ...
    <div className="min-h-screen bg-zinc-950 text-white">
      ...
      <div className="relative w-full" style={{ minHeight: 220 }}>
        <div className="absolute inset-0"
          style={{ background: 'linear-gradient(135deg, #1a0000 0%, #450a0a 45%, #7f1d1d 100%)' }} />
        <div className="absolute inset-0"
        ...
        <div className="relative z-10 px-4 pt-10 pb-5 flex flex-col gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-red-300 text-xs font-medium">
                {storeConfig.neighborhood} • {storeConfig.city}, {storeConfig.state}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mt-1">
            ...
            <span className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border"
              ...
              {storeConfig.isOpen ? '🟢 Aberto agora' : '🔴 Fechado'}
            </span>
            <span className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border border-white/15 bg-white/10 text-zinc-300">
              <Clock className="w-3 h-3" /> {storeConfig.hours}
            </span>
            <span className="flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border border-white/15 bg-white/10 text-zinc-300">
              <MapPin className="w-3 h-3" /> {storeConfig.address}
            </span>
          </div>
      ...
      <div ref={categoryBarRef}
        ...
        {categories.map(cat => {
          ...
          return (
            <button key={cat.id} data-active={isActive}
              ...
              <div key={product.id}
                ...
                <div className="flex items-start gap-3 p-4">
                  <div className="flex-1 min-w-0 flex flex-col gap-1">
                    ...
                    <button onClick={() => pedirDiretoWhatsApp(product.name, product.price)}
                ...
                <div key={item.id} className="bg-zinc-800 rounded-xl p-3 border border-zinc-700">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={() => updateQuantity(item.id, item.quantity - 1)}
                      className="w-7 h-7 rounded-md bg-zinc-700 hover:bg-zinc-600 flex items-center justify-center">
                      <Minus className="w-3 h-3" />
                    </button>
      ...
      {cartItemCount === 0 && (
        <button onClick={handleWhatsApp}
          ...
          style={{ animation: 'pulse-green 2s infinite' }} title="WhatsApp">
      ...
      {/* BOTÃO ADMIN FIXO */}
      <button onClick={() => setShowAdmin(true)}
        className="fixed bottom-6 left-5 z-40 w-12 h-12 rounded-full bg-zinc-800 hover:bg-zinc-700 border border-zinc-600 text-zinc-400 hover:text-white flex items-center justify-center shadow-lg transition-all hover:scale-105"
        title="Painel Admin">
