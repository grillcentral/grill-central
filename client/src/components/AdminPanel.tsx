...
import { X, Camera, Eye, EyeOff, Upload, Printer, RefreshCw, Settings, Lock, Trash2 } from 'lucide-react';
...
/* ─────────────────────────────────────────────────────
   Som Ding-Dong
   FIX: AudioContext compartilhado + resume() antes de tocar
   O browser suspende o contexto quando não há interação do
   usuário; ao chamar resume() antes de cada nota o som
   funciona mesmo vindo do polling (sem clique manual).
─────────────────────────────────────────────────────── */
...
function _playDingDong(ctx: AudioContext, repetition: number) {
  ...
  note(1318, t,        0.6, 0.5); // DING Mi5
  note(1047, t + 0.02, 0.6, 0.3); // Dó5
  note(880,  t + 0.7,  0.8, 0.5); // DONG Lá4
  note(659,  t + 0.72, 0.8, 0.3); // Mi4
...
function tocarNovoPedido(repetition = 0) {
  ...
  if (ctx.state === 'suspended') {
    ctx.resume().then(() => _playDingDong(ctx, repetition)).catch(() => {});
  } else {
    try { _playDingDong(ctx, repetition); } catch (e) { console.warn('Audio:', e); }
  }
}
...
export default function AdminPanel({ onClose, products }: AdminPanelProps) {
  ...
  /* ── Desbloqueia AudioContext no primeiro clique/toque ── */
  useEffect(() => {
    const unlock = () => {
      const ctx = getAudioCtx();
  ...
  /* ── Listener evento local novoPedido ── */
  useEffect(() => {
    const handler = (e: any) => {
      if (e.detail) {
  ...
  /* FIX store_config: upsert com onConflict:'id' garante insert ou update */
  async function saveStoreConfig(overrideIsOpen?: boolean) {
    const isOpenVal = overrideIsOpen !== undefined ? overrideIsOpen : storeForm.isOpen;
    try {
      ...
      console.error('saveStoreConfig:', err);
  ...
  async function deleteOrder(orderId: string) {
    ...
    try {
      ...
      lastOrderCount.current = Math.max(0, lastOrderCount.current - 1);
  ...
  /* FIX clearAllOrders: deleta por data (funciona mesmo sem policy de delete por id) */
  async function clearAllOrders() {
    if (!confirm('Tem certeza que deseja LIMPAR TODOS os pedidos?\nEsta ação não pode ser desfeita.')) return;
    try {
      // Deleta todos os pedidos com data >= 2000-01-01 (cobre qualquer pedido real)
      const { error } = await supabase
        .from('orders')
        .delete()
      ...
      console.error('clearAllOrders:', err);
      showToast('❌ Erro ao limpar — verifique permissões no Supabase');
    }
  }
  ...
  function printOrder(order: Order) {
    ...
    const pay   = order.payment_method === 'pix' ? 'PIX' : order.payment_method === 'cartao' ? 'Cartão'
...
<p>${addr}</p><p><b>Pagamento:</b> ${pay}</p><hr>
      ...
      <div className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-2xl max-h-[92vh] overflow-hidden flex flex-col shadow-2xl">
        ...
        {!isAuthenticated ? (
          <div className="flex flex-col items-center justify-center flex-1 p-8 gap-4">
            ...
            <button onClick={handleLogin} className="px-8 py-2 bg-red-600 hover:bg-red-500 text-white rounded-xl font-semibold text-sm transition">
              Entrar
            </button>
          </div>
        ) : (
              ...
              {(['pedidos', 'fotos', 'produtos', 'loja'] as const).map(tab => (
                <button
                  ...
                  className={`px-4 py-2 text-xs font-semibold capitalize whitespace-nowrap transition ${
                    activeTab === tab ? 'text-red-400 border-b-2 border-red-500 bg-zinc-900' : 'text-zinc-400 hover:text-white'
                  }`}
                >
                        ...
                        {isExpanded && (
                          <div className="border-t border-zinc-700 p-3 space-y-2">
                            <div className="grid grid-cols-2 gap-2 text-xs">
                              <div><p className="text-zinc-400">Tel</p><p className="text-white">{order.customer_phone}</p></div>
                              <div>
                                <p className="text-zinc-400">Tipo</p>
                                <p className="text-white">{order.delivery_type === 'delivery' ? '🛵 Entrega' : '🏪 Retirada'}</p>
                              </div>
                              {order.delivery_type === 'delivery' && order.address && (
                                <div className="col-span-2">
                                  ...
                                  {order.gps_lat && (
                                    <a href={`https://www.google.com/maps?q=${order.gps_lat},${order.gps_lng}`} target="_blank" rel="noreferrer" className="text-blue-400 text-xs hover:underline">
                                      📍 Ver no Maps
                                    </a>
                                  )}
                                </div>
                            ...
                            {order.observations && (
                              <div className="bg-zinc-700 rounded-lg p-2 text-xs text-zinc-300">📝 {order.observations}</div>
                            )}
                            ...
                            <div className="flex flex-wrap gap-2 pt-1">
                              {['novo', 'preparando', 'entregue', 'cancelado'].map(s => (
                                <button
                                  ...
                                  className={`px-2 py-1 rounded-lg text-xs font-medium transition ${order.status === s ? STATUS_CONFIG[s].color : 'bg-zinc-700 text-zinc-400 hover:bg-zinc-600'}`}
                                >
                                  {STATUS_CONFIG[s].label}
                                </button>
                              ))}
                              <button onClick={() => printOrder(order)} className="px-2 py-1 bg-zinc-700 hover:bg-zinc-600 text-zinc-300 rounded-lg text-xs flex items-center gap-1 transition">
                                <Printer className="w-3 h-3" /> Imprimir
                              </button>
                              <button onClick={() => deleteOrder(order.id)} className="px-2 py-1 bg-red-900 hover:bg-red-800 text-red-300 rounded-lg text-xs flex items-center gap-1 ml-auto transition">
              ...
              {activeTab === 'fotos' && (
                <div className="space-y-3">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-zinc-400 text-xs">Clique em "Foto" para enviar imagem</p>
                    <button onClick={loadImages} className="text-zinc-500 hover:text-zinc-300 text-xs flex items-center gap-1 transition">
                      <RefreshCw className="w-3 h-3" /> Recarregar
                    </button>
                  </div>
                  {safeProducts.length === 0 && <p className="text-zinc-500 text-xs text-center py-4">Nenhum produto disponível</p>}
                  {safeProducts.map(product => (
                    <div key={product.id} className="flex items-center gap-3 bg-zinc-800 rounded-xl p-3 border border-zinc-700">
                      ...
                      >
                        {uploadingId === product.id ? <Upload className="w-3 h-3 animate-spin" /> : <Camera className="w-3 h-3" />}
                      ...
                      <div key={product.id} className="bg-zinc-800 rounded-xl p-3 border border-zinc-700 space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <button
                            ...
                            className={`flex items-center gap-1 px-2 py-1 rounded-lg text-xs transition ${ov.hidden ? 'bg-red-800 text-red-300 hover:bg-red-700' : 'bg-green-800 text-green-300 hover:bg-green-700'}`}
                          >
                        ...
                        <div className="flex gap-2">
                          <input
                            ...
                            onBlur={e => { const v = parseFloat(e.target.value); if (!isNaN(v)) saveOverride(product.id, 'price', v); }}
                          />
                          <input
                            type="text"
              ...
              {activeTab === 'loja' && (
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    {/* FIX: passa o novo valor direto para saveStoreConfig para evitar closure stale */}
                    <button
                      onClick={async () => {
                        const newVal = !storeForm.isOpen;
                        setStoreForm(prev => ({ ...prev, isOpen: newVal }));
                        await saveStoreConfig(newVal);
                        showToast(newVal ? '🟢 Loja aberta!' : '🔴 Loja fechada!');
                      }}
                      className={`px-4 py-2 rounded-xl text-sm font-semibold transition ${
                        storeForm.isOpen ? 'bg-green-700 text-green-100 hover:bg-green-600' : 'bg-red-800 text-red-200 hover:bg-red-700'
                      }`}
                    >
